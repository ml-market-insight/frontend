...
const Panel: React.FC<PanelProps> = ({ selectedStocks, reset, setReset }) => {
  ...
  const handleSimulate = async () => {
    ...
    setPrediction(response.data.rendement);
    setReset(false);
    setLoading(false);
  };

  return (
    <div className="relative inline-flex flex-col divide-y divide-lightgrey rounded-lg border border-lightgrey bg-obsidian md:flex-row md:divide-x">
      <Portfolio initial={initial} setInitial={setInitial} portfolio={portfolio} />
      ...
      >
        <button
          className="flex h-11 w-32 items-center justify-center rounded-lg border border-accentdark bg-accent font-semibold text-white hover:bg-accent/80"
        ...
        >
          {loading ? (
            <div role="status">
              <svg
                aria-hidden="true"
                className="text-gray-200 dark:text-gray-600 size-4 animate-spin fill-obsidian"
                viewBox="0 0 100 101"
                fill="none"
